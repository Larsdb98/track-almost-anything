...
class LiveViewController(QObject):
    def __init__(self, view: View):
        ...
        self.roi_point_1 = None  # in live view frame
    ...
    def eventFilter(self, watched, event):
        ...
        if (
            ...
            if event.button() == Qt.LeftButton:
                ...
                self.roi_point_1_image_frame = map_live_view_to_image(
                    pixel_coords_in_live_view=self.roi_point_1,
                    last_resize_params=self.last_resize_params,
                )
                log_debug(
        ...
        if (
            ...
            if event.button() == Qt.LeftButton:
                ...
                self.roi_point_2_image_frame = map_live_view_to_image(
                    pixel_coords_in_live_view=self.roi_point_2,
                    last_resize_params=self.last_resize_params,
                )
                log_debug(
    ...
    def resize_image_for_live_view(self, image: np.ndarray) -> np.ndarray:
        ...
        marked_image = self.add_markers(
            image,
            marker1=self.roi_point_1_image_frame,
            marker2=self.roi_point_2_image_frame,
        )

        resized_image = cv2.resize(
            marked_image, (new_width, new_height), interpolation=cv2.INTER_AREA
        )
    ...
    def add_markers(
        ...
        if cnt >= 2:
            image = cv2.rectangle(
                image, marker1, marker2, color=(43, 156, 255), thickness=2
            )
