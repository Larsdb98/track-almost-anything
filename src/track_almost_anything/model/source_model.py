...
class SourceModel(QObject):
    ...
    def __init__(self):
        ...
        self.image_current_frame_id = None
    ...
    def setup_opencv_camera(
        self, camera_id: int, img_width: int = 1280, img_height: int = 720
    ) -> None:
    ...
    def release_opencv_camera(self) -> None:
    ...
    def get_file_or_directory(self, selected_path: Path) -> ImageSequenceConfig | Path:
        log_debug(f"Model :: SourceModel: Selected path: {selected_path}")

        if selected_path.is_dir():
            try:
                self.image_sequence_config = get_image_sequence_config_from_dir(
